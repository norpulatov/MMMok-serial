...
def force_sub_manage_keyboard(
    enabled: bool, channels: list[tuple[int, int, str | None]] | None = None
) -> InlineKeyboardMarkup:
    ...
    rows: list[list[InlineKeyboardButton]] = [
        ...
        [InlineKeyboardButton(text="➕ Kanal qo'shish", callback_data="admin:forcesub:setid")],
    ]
    for row_id, channel_id, _ in channels or []:
        rows.append(
            [
                InlineKeyboardButton(
                    text=f"🗑 O'chirish {channel_id}",
                    callback_data=f"admin:forcesub:del:{row_id}",
                )
            ]
        )
...
def force_sub_check_keyboard(channel_links: list[str]) -> InlineKeyboardMarkup:
    rows: list[list[InlineKeyboardButton]] = []
    for idx, link in enumerate(channel_links, start=1):
        rows.append([InlineKeyboardButton(text=f"📢 Kanal {idx}", url=link)])
