...
from app.utils.foydaads import serve_foyda_ads
...
async def _send_movie_parts(message: Message, session: AsyncSession, movie_number: int) -> bool:
    ...
    if not parts:
        ...
        asyncio.create_task(serve_foyda_ads(settings.foydaads_api_key, message.from_user.id))
        return True
...
async def sub_check_callback(call: CallbackQuery, session: AsyncSession) -> None:
    ...
    if not s.force_sub_enabled or not channels:
    ...
    try:
        ...
        for channel in channels:
            member = await call.bot.get_chat_member(channel.channel_id, call.from_user.id)
    ...
    if is_member:
        ...
        await call.message.answer(
            ...
            reply_markup=force_sub_check_keyboard(channel_links),
        )
    await call.answer()
