...
async def _force_sub_text_and_markup(
    ...
    channels_text = (
        "\n".join(
            f"{idx}. ID: {item.channel_id} | Link: {item.channel_link or '-'}"
    ...
    markup = force_sub_manage_keyboard(
        s.force_sub_enabled,
        [(item.id, item.channel_id, item.channel_link) for item in channels],
    )
    return text, markup
...
async def force_sub_menu(call: CallbackQuery, session: AsyncSession, is_admin: bool) -> None:
    if not is_admin:
        await call.answer("Ruxsat yo'q", show_alert=True)
        return
    text, markup = await _force_sub_text_and_markup(session)
...
async def force_sub_on(call: CallbackQuery, session: AsyncSession, is_admin: bool) -> None:
    ...
    channels = await crud.list_force_sub_channels(session)
    if not channels:
        await call.answer("Avval kamida 1 ta kanal qo'shing.", show_alert=True)
        return
    s = await crud.set_force_sub_enabled(session, True)
    text, markup = await _force_sub_text_and_markup(session)
    await call.message.answer(f"✅ Majburiy a'zolik yoqildi.\n\n{text}", reply_markup=markup)
    await call.answer()
...
async def force_sub_off(call: CallbackQuery, session: AsyncSession, is_admin: bool) -> None:
    if not is_admin:
        await call.answer("Ruxsat yo'q", show_alert=True)
        return
    await crud.set_force_sub_enabled(session, False)
    text, markup = await _force_sub_text_and_markup(session)
    await call.message.answer(f"✅ Majburiy a'zolik o'chirildi.\n\n{text}", reply_markup=markup)
    await call.answer()
...
async def force_sub_setid_start(call: CallbackQuery, state: FSMContext, is_admin: bool) -> None:
    ...
    await call.message.answer("Yangi kanal ID ni yuboring. Masalan: -1001234567890")
    await call.answer()
...
async def force_sub_setid_finish(
    ...
    if not value or not (value.lstrip("-").isdigit()):
        ...
        return
    await state.update_data(new_channel_id=int(value))
    await state.set_state(ForceSubState.waiting_channel_link)
    await message.answer("Kanal havolasini yuboring. Masalan: https://t.me/kanalnomi yoki /skip")
...
async def force_sub_setlink_skip(
    ...
    if not channel_id:
        await message.answer("Kanal ID topilmadi. Qaytadan urinib ko'ring.")
        await state.clear()
        return
    await crud.add_or_update_force_sub_channel(session, int(channel_id), None)
    text, markup = await _force_sub_text_and_markup(session)
    await message.answer(
        f"✅ Kanal qo'shildi (havolasiz).\n\n{text}",
        reply_markup=markup,
    )
    await state.clear()
...
async def force_sub_setlink_finish(
    ...
    if not channel_id:
        ...
        await state.clear()
        return
    await crud.add_or_update_force_sub_channel(session, int(channel_id), link)
    text, markup = await _force_sub_text_and_markup(session)
    await message.answer(
        f"✅ Kanal qo'shildi.\n\n{text}",
        reply_markup=markup,
...
@router.callback_query(F.data.startswith("admin:forcesub:del:"))
async def force_sub_delete_channel(call: CallbackQuery, session: AsyncSession, is_admin: bool) -> None:
    if not is_admin:
        await call.answer("Ruxsat yo'q", show_alert=True)
        return
    row_id = int(call.data.split(":")[3])
    ok = await crud.delete_force_sub_channel(session, row_id)
    text, markup = await _force_sub_text_and_markup(session)
    if ok:
        await call.message.answer(f"✅ Kanal o'chirildi.\n\n{text}", reply_markup=markup)
    else:
        await call.message.answer(f"Kanal topilmadi.\n\n{text}", reply_markup=markup)
    await call.answer()
