...
async def _ensure_missing_columns() -> None:
    ...
    async with engine.begin() as conn:
        ...
        # Backward compatibility for forced subscription:
        # copy old single bot_settings channel fields into new force_sub_channels table.
        try:
            if settings.database_url.startswith("postgresql"):
                result = await conn.execute(
                    text(
                        ...
                        WHERE table_name = 'force_sub_channels'
                        """
                    )
                )
                has_force_sub_channels = int(result.scalar() or 0) > 0
            else:
                result = await conn.execute(
                    text(
                        """
                        SELECT COUNT(*) FROM sqlite_master
                        WHERE type='table' AND name='force_sub_channels'
                        """
                    )
                )
                has_force_sub_channels = int(result.scalar() or 0) > 0

            if has_force_sub_channels:
                result = await conn.execute(text("SELECT COUNT(*) FROM force_sub_channels"))
                force_sub_count = int(result.scalar() or 0)
                if force_sub_count == 0:
                    result = await conn.execute(
                        text(
                            """
                            SELECT force_sub_channel_id, force_sub_channel_link
                            FROM bot_settings
                            WHERE id = 1
                            """
                        )
                    ...
                    if row and row[0]:
                        await conn.execute(
                            text(
                                """
                                INSERT INTO force_sub_channels (channel_id, channel_link)
                                VALUES (:channel_id, :channel_link)
                                """
                            ),
                            {"channel_id": int(row[0]), "channel_link": row[1]},
                        )
                        logger.info("Eski force_sub_channel_id/link force_sub_channels jadvaliga ko'chirildi.")
        except Exception:
            logger.exception("force_sub kanal migratsiyasi bajarilmadi.")
